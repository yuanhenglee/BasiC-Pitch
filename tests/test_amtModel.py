...
def plot_hm(datas):
    ...
    for i, prefix in enumerate(["Baseline", "Ours"]):
        for j, suffix in enumerate(["contour", "note", "onset"]):
            ...
            plt.imshow(
                v.T, cmap="coolwarm", vmin=0, vmax=1, aspect="auto", origin="lower"
            )
    plt.tight_layout()
...
def test_inference(vis=False):
    ...
    gold = run_inference("data/Undertale-Megalovania.wav", ICASSP_2022_MODEL_PATH)
    print(f"Elapsed time: {time.time() - start_time}")
    ...
    for output in ["onset", "contour", "note"]:
        assert (
            plot_dict["Baseline " + output].shape == plot_dict["Ours " + output].shape
        )
        assert np.allclose(
            plot_dict["Baseline " + output], plot_dict["Ours " + output], atol=1e-3
        )
